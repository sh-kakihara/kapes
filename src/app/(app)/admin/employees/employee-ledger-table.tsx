...
function makeColumns(refDate: Date): ColumnDef<Row>[] {
  ...
  return [
    ...
    faceted("hire_date",       "入社年月日", (r) => fmtDate(rec(r, "hire_date") as string | null), { noFilter: true }),
    faceted("years",           "勤続年数",  (r) => calcYears(rec(r, "hire_date") as string | null, refDate), { noFilter: true }),
    faceted("birth_date",      "生年月日",  (r) => fmtDate(rec(r, "birth_date") as string | null), { noFilter: true }),
    faceted("age",             "年齢",      (r) => calcAge(rec(r, "birth_date") as string | null, refDate),  { noFilter: true }),
    ...
    // 今年度
    faceted("prev_annual_income",      "前年度年収", (r) => fmt(rec(r, "prev_annual_income") as number), { noFilter: true }),
    faceted("curr_base_salary",        "基本給",   (r) => fmt(rec(r, "curr_base_salary") as number),   { noFilter: true }),
    faceted("curr_position_allowance", "役職手当", (r) => fmt(rec(r, "curr_position_allowance") as number), { noFilter: true }),
    faceted("curr_salary_increase",    "昇給額",   (r) => fmt(rec(r, "curr_salary_increase") as number), { noFilter: true }),
    ...
    faceted("curr_summer_bonus",         "夏期賞与額", (r) => fmt(rec(r, "curr_summer_bonus") as number),  { noFilter: true }),
    faceted("curr_summer_bonus_add",     "夏期精勤手当", (r) => { const v = rec(r, "curr_summer_bonus_add"); return v != null ? fmt(v as number) : ""; }, { noFilter: true }),
    faceted("curr_summer_payment",       "夏期支給額", (r) => { const v = rec(r, "curr_summer_payment"); return v != null ? fmt(v as number) : ""; }, { noFilter: true }),
    ...
    faceted("curr_winter_bonus",         "冬期賞与額", (r) => fmt(rec(r, "curr_winter_bonus") as number),  { noFilter: true }),
    faceted("curr_winter_bonus_add",     "冬期精勤手当", (r) => { const v = rec(r, "curr_winter_bonus_add"); return v != null ? fmt(v as number) : ""; }, { noFilter: true }),
    faceted("curr_winter_payment",       "冬期支給額", (r) => { const v = rec(r, "curr_winter_payment"); return v != null ? fmt(v as number) : ""; }, { noFilter: true }),
    faceted("curr_notes", "備考", (r) => (rec(r, "curr_notes") as string) ?? "", { noFilter: true }),
  ];
}
